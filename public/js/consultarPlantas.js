      ...
      querySnapshot.forEach((doc) => {
        ...
        tabla.innerHTML += `
                <tr>  
                        ...
                        <td>${doc.data().tipo}</td>
                        <td><a onclick="muestraDetalles('${
                          doc.id
                        }')" data-bs-toggle="modal" data-bs-target="#verPlantaModal"><img src="img/ver.svg" class="iconos-tam"></a></td>
                        <td><a onclick="editarPlanta('${
                          doc.id
                        }')" data-bs-toggle="modal" data-bs-target="#modificarPlantaModal"><img src="img/editar.svg" class="iconos-tam"></a></td>
                        <td><a><img src="img/eliminar.svg" class="iconos-tam" onclick="eliminar('${
                          doc.id
                        }')"></a></td>
                </tr>
            `;
  ...
  planta
    ...
    .then((doc) => {
      if (doc.exists) {
        ...
        modalInfo.innerHTML += `
                    <h2>${doc.data().tipo}</h2>
                    <p style=" font-weight: bold;">Especie:</p><p>${
                      doc.data().especie
                    }</p>
                    <div><p style="font-weight:bold ">Descripcion de la planta:</p> <p style="font-size: small;">${
                      doc.data().descripcion
                    }</p></div>
                    <div><p style="font-weight:bold ">Lugar de plantación: </p><p style="font-size: small;">${
                      doc.data().lugar
                    }</p></div>
                    <div><p style="font-weight:bold ">Temporada: </p><p style="font-size: small;">${
                      doc.data().temp
                    }</p></div>
                    <div><p style="font-weight:bold ">Usos: </p><p style="font-size: small;">${
                      doc.data().usos
                    }</p></div>
                    <div><p style="font-weight:bold ">Ubicación en casa:</p> <p style="font-size: small;">${
                      doc.data().ubicacion
                    }</p></div>
                    <div><p style="font-weight:bold ">Fecha en que fue plantada:</p><p style="font-size: small;"> ${
                      doc.data().fechaPlantacion
                    }</p></div>
                    <div><p style="font-weight:bold ">Notas sobre el cuidado de la planta:</p> <p style="font-size: small;"> ${
                      doc.data().notas
                    }</p></div>
            `;
        imagenes.innerHTML = "";
        imagenes.innerHTML += `
                <img class="img-fluid d-block small-preview" src="${
                  doc.data().img1
                }">
            `;
      } else {
